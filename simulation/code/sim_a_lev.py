"""Simulation A: sweep over maximum perturbation level"""
...
max_levels = [4, 3.6, 3.2, 2.8, 2.4, 2, 1.6, 1.2, 0.8, 0.4, 0]
selected_levels = [4, 2, 0]

PKL_PATH = "sim_a_lev.pkl"
CSV_PATH = "sim_a_lev_selector_acc.csv"
...
def sample_training_envs(train_levels):
    envs, xs, ys = [], [], []
    for pt in perturb_types:
        for lvl in train_levels:
            X, Y = generate_env_data(pt, float(lvl), n_train, noise_std=TRAIN_NOISE)
...
def run_condition(max_level):
    train_levels = np.linspace(0, float(max_level), int(n_env))
    ...
    for s in range(n_sim):
        X_pool, Y_pool, envs = sample_training_envs(train_levels)
...
def load_or_compute():
    if os.path.exists(PKL_PATH):
        ...
        if isinstance(obj, dict) and 'results' in obj:
            obj = obj['results']
        if isinstance(obj, dict) and TRAIN_NOISE in obj and isinstance(obj[TRAIN_NOISE], dict):
            return obj[TRAIN_NOISE]
        if isinstance(obj, dict) and obj and isinstance(next(iter(obj.values())), dict) and 'selector_acc' in next(iter(obj.values())):
            return obj

    results = {float(ml): run_condition(ml) for ml in max_levels}
...
with open(CSV_PATH, 'w', newline='') as f:
    w = csv.writer(f)
    w.writerow(['max_level', 'u', 'u_se', 'c', 'c_se'])
    for ml in max_levels:
        u_mean, u_se = results[float(ml)]['selector_acc']['eacs']
        c_mean, c_se = results[float(ml)]['selector_acc']['causal']
        w.writerow([ml, _fmt(u_mean), _fmt(u_se), _fmt(c_mean), _fmt(c_se)])


baseline_ref = float(max_levels[0])
...
for include_constrained in [False, True]:
    ...
    for j, pt in enumerate(perturb_types):
        ...
        base_c2 = results[baseline_ref]['baseline']['mean_b2'][pt]
        se_c2 = results[baseline_ref]['baseline']['se_b2'][pt]
        base_c2x = results[baseline_ref]['baseline']['mean_b23'][pt]
        se_c2x = results[baseline_ref]['baseline']['se_b23'][pt]
        ...
        for k, ml in enumerate(max_levels):
            cg = plt.cm.Greens(0.8 - 0.6 * k / len(max_levels))
            ma = results[float(ml)]['eacs']['mean'][pt]
            se = results[float(ml)]['eacs']['se'][pt]
            lbl = f'{float(ml):.1f} lev' if (j == 0 and ml in selected_levels) else '_nolegend_'
        ...
        if include_constrained:
            for k, ml in enumerate(max_levels):
                cb = plt.cm.Blues(0.8 - 0.6 * k / len(max_levels))
                mc = results[float(ml)]['eacs_constrained']['mean'][pt]
                se = results[float(ml)]['eacs_constrained']['se'][pt]
                lbl = f'cau {float(ml):.1f} lev' if (j == 0 and ml in selected_levels) else '_nolegend_'
...
x_vals = sorted([float(x) for x in max_levels])

means_u = [results[ml]['selector_acc']['eacs'][0] for ml in x_vals]
ses_u = [results[ml]['selector_acc']['eacs'][1] for ml in x_vals]
means_c = [results[ml]['selector_acc']['causal'][0] for ml in x_vals]
ses_c = [results[ml]['selector_acc']['causal'][1] for ml in x_vals]

col_u = plt.cm.Greens(0.6)
col_c = plt.cm.Blues(0.6)

h1 = plt.plot(x_vals, means_u, marker='o', color=col_u, linewidth=2)[0]
plt.fill_between(x_vals, np.array(means_u) - 1.96 * np.array(ses_u), np.array(means_u) + 1.96 * np.array(ses_u), color=col_u, alpha=0.1)

h2 = plt.plot(x_vals, means_c, marker='s', color=col_c, linewidth=2)[0]
plt.fill_between(x_vals, np.array(means_c) - 1.96 * np.array(ses_c), np.array(means_c) + 1.96 * np.array(ses_c), color=col_c, alpha=0.1)

plt.xlabel('Max Perturbation Level')
