"""Simulation A: sweep over sample size per environment"""
...
n_env = 100
sample_sizes = [100, 90, 80, 70, 60, 50, 40, 30, 20, 15, 10, 5]
selected_sizes = [100, 20, 5]

n_test, n_sim = 100, 1000

PKL_PATH = "sim_a_sam.pkl"
CSV_PATH = "sim_a_sam_selector_acc.csv"
...
def sample_training_envs(train_levels, n_train, noise_std):
    envs, xs, ys = [], [], []
    for pt in perturb_types:
        for lvl in train_levels:
            X, Y = generate_env_data(pt, float(lvl), int(n_train), noise_std=noise_std)
...
def run_condition(train_size, noise_std):
    ...
    for s in range(n_sim):
        X_pool, Y_pool, envs = sample_training_envs(train_levels, n_train=int(train_size), noise_std=noise_std)
...
def load_or_compute():
    ...
    for noise in noise_levels:
        ...
        for train_size in sample_sizes:
            out = run_condition(train_size, noise_std=float(noise))
            res_noise['baseline'][train_size] = out['baseline']
            res_noise['eacs'][train_size] = out['eacs']
            res_noise['eacs_constrained'][train_size] = out['eacs_constrained']
            res_noise['selector_acc'][train_size] = out['selector_acc']
...
with open(CSV_PATH, 'w', newline='') as f:
    ...
    for train_size in sample_sizes:
        row = [train_size]
        for s in noise_levels:
            u_mean, u_se = results[float(s)]['selector_acc'][train_size]['eacs']
            c_mean, c_se = results[float(s)]['selector_acc'][train_size]['causal']
            row += [_fmt(u_mean), _fmt(u_se), _fmt(c_mean), _fmt(c_se)]
        w.writerow(row)


baseline_ref = sample_sizes[0]
...
for include_constrained in [False, True]:
    ...
    for i, noise in enumerate(noise_levels):
        ...
        for j, pt in enumerate(perturb_types):
            ...
            for k, train_size in enumerate(sample_sizes):
                cg = plt.cm.Greens(0.8 - 0.6 * k / len(sample_sizes))
                ma = res['eacs'][train_size]['mean'][pt]
                se = res['eacs'][train_size]['se'][pt]
                lbl = f'{train_size} sam' if (i == 0 and j == 0 and train_size in selected_sizes) else '_nolegend_'
            ...
            if include_constrained:
                for k, train_size in enumerate(sample_sizes):
                    cb = plt.cm.Blues(0.8 - 0.6 * k / len(sample_sizes))
                    mc = res['eacs_constrained'][train_size]['mean'][pt]
                    se = res['eacs_constrained'][train_size]['se'][pt]
                    lbl = f'cau {train_size} sam' if (i == 0 and j == 0 and train_size in selected_sizes) else '_nolegend_'
...
for idx, noise in enumerate(noise_levels):
    ...
    means = [acc[s]['eacs'][0] for s in x_vals]
    ses = [acc[s]['eacs'][1] for s in x_vals]
...
for idx, noise in enumerate(noise_levels):
    acc = results[float(noise)]['selector_acc']
    means = [acc[s]['causal'][0] for s in x_vals]
    ses = [acc[s]['causal'][1] for s in x_vals]
...
plt.xlabel('Samples per Environment')
